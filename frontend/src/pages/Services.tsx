...
export default function Services() {
    ...
    const [myData, setData] = useState<File>()
    const handleSubmit = async (e: { preventDefault: () => void }) => {
        e.preventDefault()
        try {
            await axiosAPI.post('/postOrder', myData)
            toast.success('Good')
        } catch (error) {
            toast.error('Bad')
        }
                ...
                <div className="flex flex-col mt-12 gap-7">
                    ...
                    <div className=" flex flex-col gap-7">
                        <div className=" w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                            <a href="#">
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Ajoutez un Service</h5>
                            </a>
                            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Téléchargez un cahier de charge contient les details de service. Si vous n'avez pas un cahier charge, vou pouvez demandez votre service par text</p>
